...
module.exports.handler = async (event, context) => {
    ...
    try{
        ...
        if( event.body.type == "answer" ){
            ...
            let user = await db.collection( "admins" ).findOne( query )
            client.close()
            ...
            if( value ){
                ...
                let html = `<div> <h2 style="text-align: center; color: #8396d3; font-family: Arial;">Notificación de operación<br/></h2> <p style="text-align: center;">Apreciable ${value.name}. Le informamos que el proceso de recuperación en su cuenta ha finalizado en este momento y su contraseña ha sido modificada exitosamente.</p><p style="text-align: center;"> Si no reconoce ésta operación, por favor envíenos un correo a <strong>soporte@investus.mx</strong>. </p></div>`
                let params = {
                    html: html,
                    from: "Investus <no-reply@investus.mx>",
                    to: value.username,
                    subject: "Notificación de operación | Investus"
                }
