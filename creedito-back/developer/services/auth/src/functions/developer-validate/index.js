...
module.exports.handler = async (event, context) => {
    ...
    try{
        ...
        let setter = { $set: {verified: true, verificationHash: null} }
        let query = { verificationHash: hash }
        let response = await db.collection( "developers" ).updateOne( query, setter )
        ...
        if( response.result.n === 1 ){
            ...
            return {
                statusCode: 301,
                headers: {
                    Location: appURL + "validated-account"
                }
            }
            ...
            return {
                statusCode: 301,
                headers: {
                    Location: appURL + "validate-error"
                }
            }
        }
    }catch(e) {
