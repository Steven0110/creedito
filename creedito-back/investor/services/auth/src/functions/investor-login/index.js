...
module.exports.handler = async (event, context) => {
    ...
    try{
        ...
        let user = await db.collection( "investors" ).findOne( query, {projection: project} )
        if( user ){
            ...
            if( user.tfa === true ){
                if( event.body.otp ){
                    ...
                    if( verified ){
                        ...
                        await db.collection( "investors" ).updateOne( tmpQuery, setter )
            ...
            const { value } = await db.collection( "investors" ).findOneAndUpdate( tmpQuery, setter, options )
