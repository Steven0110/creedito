...
module.exports.handler = (event, context, callback) => {
    ...
    .then(() => {
        ...
        return db.collection( "developers" ).findOne( query, {projection: project} )
    })
    .then(user => {
        client.close()
        if( user && user.otpToken ){
            let otp = event.headers.Authorization
            let verified = speakeasy.totp.verify({ secret: user.otpToken, encoding: 'base32', token: otp })
            if( verified )
